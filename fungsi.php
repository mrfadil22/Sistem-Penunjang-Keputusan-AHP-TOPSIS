...
{
    include('config.php');
    $query  = "SELECT id_kriteria FROM tab_kriteria ORDER BY id_kriteria";
...
{
    include('config.php');
    $query  = "SELECT id_alternatif FROM tab_alternatif ORDER BY id_alternatif";
    $result = mysqli_query($koneksi, $query);
...
{
    include('config.php');
    $query  = "SELECT nama_kriteria FROM tab_kriteria ORDER BY id_kriteria";
    $result = mysqli_query($koneksi, $query);
...
{
    include('config.php');
    $query  = "SELECT nama_alternatif FROM tab_alternatif ORDER BY id_alternatif";
    $result = mysqli_query($koneksi, $query);
...
{
    include('config.php');
    $query  = "SELECT count(*) FROM tab_alternatif";
    $result = mysqli_query($koneksi, $query);
    while ($row = mysqli_fetch_array($result)) {
        $jmlData = $row[0];
    }
...
{
    include('config.php');
    $query  = "SELECT count(*) FROM tab_kriteria";
    $result = mysqli_query($koneksi, $query);
    while ($row = mysqli_fetch_array($result)) {
        $jmlData = $row[0];
    }
...
function tambahDataNilmat($tabel, $alternatif, $kriteria, $poin)
{
    include('config.php');

    $query     = "INSERT INTO tab_topsis ('id_alternatif','id_kriteria','nilai') VALUES ('$alternatif', '$kriteria', $poin')";
    $tambah    = mysqli_query($koneksi, $query);
    if (!$tambah) {
        echo "Gagal mmenambah data" . $tabel;
        exit();
...
function deleteKriteria($id)
{
    ...
    $query     = "DELETE FROM tab_kriteria WHERE id_kriteria=$id";
...
function deleteAlternatif($id)
{
    ...
    $query     = "DELETE FROM tab_alternatif WHERE id_alternatif=$id";
    mysqli_query($koneksi, $query);
}

function inputRangking($nama, $V1)
{
    include('config.php');

    $query = "SELECT * FROM tab_rangking WHERE nama_alternatif=$nama And nilai=$V1";
    $result = mysqli_query($koneksi, $query);
    ...
    if (mysqli_num_rows($result) == 0) {
        $query = "INSERT INTO tab_rangking ('nama_alternatif','nilai') VALUES ('$nama''$V1')";
    } else {
        $query = "UPDATE tab_rangking SET nilai=$V1 where nama_kriteria = $nama";
    }
...
function inputKriteriaPV($id_kriteria, $pv)
{
    ...
    $query2 = "SELECT * FROM tab_kriteria WHERE id_kriteria=$id_kriteria";
    ...
    if (mysqli_num_rows($result2) == 0) {
        $query = "INSERT INTO tab_kriteria (id_kriteria, bobot) VALUES ($id_kriteria, $pv)";
    } else {
        $query = "UPDATE tab_kriteria SET bobot=$pv WHERE id_kriteria=$id_kriteria";
    }
    $result2 = mysqli_query($koneksi, $query2);
    if (!$result2) {
        echo "Gagal memasukkan / update nilai priority vector kriteria";
...
function inputDataPerbandinganAlternatif($alternatif1, $alternatif2, $pembanding, $nilai)
{
    ...
    $query  = "SELECT * FROM perbandingan_alternatif WHERE alternatif1 = $id_alternatif1 AND kriteria1 = $id_alternatif2 AND pembanding = $id_pembanding";
    $result = mysqli_query($koneksi, $query);
    ...
    // jika telah ada maka diupdate
    if (mysqli_num_rows($result) == 0) {
        $query = "INSERT INTO perbandingan_alternatif (alternatif1,kriteria1,pembanding,nilai) VALUES ($id_alternatif1,$id_alternatif2,$id_pembanding,$nilai)";
    } else {
        $query = "UPDATE perbandingan_alternatif SET nilai=$nilai WHERE alternatif1=$id_alternatif1 AND kriteria1=$id_alternatif2 AND pembanding=$id_pembanding";
    }
...
function getNilaiPerbandinganAlternatif($alternatif1, $alternatif2, $pembanding)
{
    ...
    $query  = "SELECT nilai FROM perbandingan_alternatif WHERE alternatif1 = $id_alternatif1 AND kriteria1 = $id_alternatif2 AND pembanding = $id_pembanding";
    $result = mysqli_query($koneksi, $query);
...
<?php
// menampilkan tabel perbandingan bobot
function showTabelPerbandinganAlternatif($jenis, $alternatif, $kriteria)
{
    include('config.php');

    if ($alternatif == 'tab_alternatif') {
    ...
    if ($kriteria == 'tab_kriteria') {
        $n2 = getJumlahKriteria();
    }

    $query2 = "SELECT nama FROM $kriteria ORDER BY id";
    $result2   = mysqli_query($koneksi, $query2);
    if (!$result2) {
        echo "Error koneksi database!!!";
        exit();
...
?>
    ...
    <form class="ui form" action="proses2.php" method="post">
        <table class="ui celled selectable collapsing table">
            <thead>
                <tr>
                    <th colspan="2">pilih yang lebih penting</th>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="field">
                                    <div class="ui radio checkbox">
                                        <input name="pilih<?php echo $urut ?>" value="2" class="hidden" type="radio">
                                        <label><?php echo $pilihan2[$y]; ?></label>
                                    </div>
                                </div>
                            </td>
                            <td>
